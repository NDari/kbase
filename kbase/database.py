...
Moore-Hodgson algorithm is a scheduling algorithm which tries to reduce the number of late tasks. For this we start by sorting tasks by earliest due date. If it looks like we wont make a deadline, we remove/delete the most time-consuming task from the queue and repeat. If we cant delete, we can just put these tasks at the end of the queue since they will be late anyway
